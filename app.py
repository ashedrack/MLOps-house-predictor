...
with col1:
    ...
    year_built = st.number_input(
        "Year Built", min_value=1800, max_value=2025, value=1961
    )
    total_bsmt_sf = st.number_input(
        "Total Basement Area (sq ft)", min_value=0, value=850
    )
    gr_liv_area = st.number_input(
        "Above Ground Living Area (sq ft)", min_value=0, value=1710
    )
...
with col2:
    ...
    bedroom_abvgr = st.number_input(
        "Number of Bedrooms Above Ground", min_value=0, value=3
    )
    kitchen_abvgr = st.number_input(
        "Number of Kitchens Above Ground", min_value=0, value=1
    )
...
if st.button("Predict Price", type="primary"):
    try:
        ...
        if not os.path.exists(model_path):
            st.error(
                "Model not found. Please make sure you have trained the model first."
            )
            st.stop()
        ...
        importance_df = pd.DataFrame(
            {"Feature": importance_data.keys(), "Importance": importance_data.values()}
        )
